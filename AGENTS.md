...
- **Node.js** — version pinned in `.nvmrc` (currently v24.11.0). Installed via nvm and set as the nvm default. **You must use the pinned Node 24 for the frontend build:** `yarn start` (and plugin webpack builds) load `.ts` webpack configs that rely on Node's native type stripping and **fail on Node 22** with `ERR_UNKNOWN_FILE_EXTENSION ".ts"`. **PATH gotcha:** the infra injects `/exec-daemon/node` (Node 22) ahead of nvm in PATH, so a plain non-login shell resolves `node` to 22. `~/.bashrc` has been patched to prepend the nvm default bin ahead of `/exec-daemon`, so **login shells (tmux sessions, `bash -lc '...'`) resolve Node 24**. Always run `yarn start` / `yarn` / `jest` / webpack via a login shell (tmux or `bash -lc`). `yarn install` itself works under either Node, but the frontend build does not. If `.nvmrc` bumps Node, `nvm install <ver> && nvm alias default <ver>`.
